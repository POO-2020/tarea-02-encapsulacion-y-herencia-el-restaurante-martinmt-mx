...
export default class Pedido{
    ...
    constructor(fecha, hora, cliente){
        ...
        this.elementosPedido = new Array()
    }
    agregarElemento(elementoPedido){
        this.elementosPedido.push(elementoPedido)
    }
    listarElementos(){
        this.elementosPedido.forEach((elementoPedido, i) => {
    ...
    getNumeroElementos(){
        return `${this.elementosPedido.length}`
    }
    getProductos(){
        let i = 0
        this.elementosPedido.forEach(elementoPedido => {i = elementoPedido.cantidad + i})
        return i
    }
    getCostoTotal(){
        let i = 0
